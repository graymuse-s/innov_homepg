...
import Loader from "./InitialAnimation.jsx";
...
function Model({ path, position, scale = 1, rotation = [0, 0, 0] }) {
  ...
  // SkeletonUtils.clone reuses material references instead of deep-cloning
  // preventing extra shader recompilation per clone
...
// ─────────────────────────────────────────────
// STAGGERED SCENE — reveals 1 model every 300ms
// Equal time gap between every model appearing
// ─────────────────────────────────────────────
function StaggeredScene({
  getCornerPos,
  cowsData,
  horsesData,
  sheepData,
  ...
  const INTERVAL_MS = 300; // ← change this to make gaps shorter or longer
  const TOTAL = 46;

  // Tick up visibleCount every INTERVAL_MS — equal gap between each model
  useEffect(() => {
  ...
  // After each model is revealed, pre-compile its shaders immediately
  // so the GPU never has to compile on the fly = no stutter
  useEffect(() => {
    const id = requestAnimationFrame(() => {
      gl.compile(scene, camera);
    });
    return () => cancelAnimationFrame(id);
  }, [visibleCount, gl, scene, camera]);

  const show = (n) => visibleCount >= n;
...
export default function City({ onSelectDome }) {
  ...
  // Fires every time a new model is added to the scene.
  // gl.compile() pre-builds all GPU shaders BEFORE the renderer
  // needs them — eliminating the 15-20s stutter spike after load.
  useEffect(() => {
    const id = requestAnimationFrame(() => {
      gl.compile(scene, camera);
    });
  ...
  return (
    <group>
      ...
      {/* ── STAGGERED LOADER: 1 model every 300ms, equal gaps ── */}
      <StaggeredScene
