...
function InstancedRoadTrees({ angle, radius }) {
  ...
  const treeData = useMemo(() => {
    ...
    for (let i = 0; i < treeCount; i++) {
      const zPos = i * (segmentLength / (treeCount - 1)) - segmentLength / 2;
...
function TreeCluster({ position, count = 20, spread = 15 }) {
  ...
  const treeData = useMemo(() => {
    return Array.from({ length: count }).map(() => ({
      pos: [(Math.random() - 0.5) * spread, 0, (Math.random() - 0.5) * spread],
      scale: 3 + Math.random() * 3,
      ...
      <Suspense fallback={null}>
        ...
        <group position={getCornerPos(5)}>
          ...
          <AllTreeClusters
            clusters={[
              {
                position: [55 + getCornerPos(5)[0], 1, 55 + getCornerPos(5)[2]],
                count: 20,
                spread: 12,
              },
              {
              ...
              {
                position: [getCornerPos(5)[0], 0, getCornerPos(5)[2]],
                count: 15,
                spread: 40,
              },
            ]}
